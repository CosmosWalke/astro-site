...
interface MobileMenuProps {
  sections: Array<{ 
    ...
    href: string; 
    id: string;
    isPage?: boolean;  // ← добавляем опциональное поле
  }>
  onNavigate: (sectionId: string, isPage?: boolean, href?: string) => void  // ← обновляем тип функции
}
...
export function MobileMenu({ sections, onNavigate }: MobileMenuProps) {
  ...
  const textPosition = isMobile
    ? { left: '27%', top: '20%' }     // мобильные: 5% слева, 12% сверху
    : { left: '35%', top: '18%' }     // ПК: 10% слева, 18% сверху
...
>
                    {isActive ? (
                      ...
                      <TextScramble 
                        ...
                        className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-wide text-white group-hover:text-[#00d4ff] transition-colors duration-300"
                      />
                    )}
